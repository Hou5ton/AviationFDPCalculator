...
class PlanningTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.crew_member_combo.currentIndexChanged.connect(self.on_crew_member_changed)
        input_layout.addRow("Член экипажа:", self.crew_member_combo)

        # Выбор воздушного судна
        self.aircraft_combo = QComboBox()
        self.aircraft_combo.currentIndexChanged.connect(self.on_aircraft_changed)
        input_layout.addRow("Воздушное судно:", self.aircraft_combo)

        # Основное место базирования (автозаполняется при выборе экипажа)
        self.home_base_label = QLabel("Не выбрано")
        input_layout.addRow("Основное место базирования:", self.home_base_label)

        # Класс отдыха ВС (автозаполняется при выборе ВС)
        self.rest_class_label = QLabel("Не выбрано")
        input_layout.addRow("Класс отдыха ВС:", self.rest_class_label)
        ...
        self.selected_crew_member_id = None
        self.selected_aircraft_id = None

    def on_crew_member_changed(self, index):
        """Обработчик изменения выбора члена экипажа"""
        if index >= 0:
            self.selected_crew_member_id = self.crew_member_combo.currentData()
            # Получаем информацию о члене экипажа
            conn = sqlite3.connect('fdp_data.db')
            cursor = conn.cursor()
            cursor.execute("SELECT home_base FROM crew_members WHERE id = ?", (self.selected_crew_member_id,))
            result = cursor.fetchone()
            conn.close()

            if result:
                self.home_base_label.setText(result[0])

    def on_aircraft_changed(self, index):
        """Обработчик изменения выбора воздушного судна"""
        if index >= 0:
            self.selected_aircraft_id = self.aircraft_combo.currentData()
            # Получаем информацию о воздушном судне
            conn = sqlite3.connect('fdp_data.db')
            cursor = conn.cursor()
            cursor.execute("SELECT rest_facility_class FROM aircrafts WHERE id = ?", (self.selected_aircraft_id,))
            result = cursor.fetchone()
            conn.close()
            ...
            if result:
                rest_class = result[0]
                if rest_class == 1:
                    self.rest_class_label.setText("1 класс (спальное место)")
                elif rest_class == 2:
                    self.rest_class_label.setText("2 класс (удобное кресло)")
                elif rest_class == 3:
                    self.rest_class_label.setText("3 класс (стандартное кресло)")
                else:
                    self.rest_class_label.setText("Не предусмотрен")
    ...
    def calculate_plan(self):
        ...
        try:
            ...
            if not self.selected_crew_member_id:
                QMessageBox.warning(self, "Предупреждение", "Выберите члена экипажа!")
                return

            if not self.selected_aircraft_id:
                QMessageBox.warning(self, "Предупреждение", "Выберите воздушное судно!")
                return
            ...
            # Получаем информацию о воздушном судне
            cursor.execute("SELECT rest_facility_class FROM aircrafts WHERE id = ?", (self.selected_aircraft_id,))
            rest_facility_class = cursor.fetchone()[0]
            conn.close()
            ...
            result += f"Класс отдыха ВС: {self.rest_class_label.text()}\n"
            result += f"Основное место базирования: {self.home_base_label.text()}\n"
            ...
            # Если предусмотрен отдых в полете, рассчитываем его
            if rest_in_flight and rest_facility_class:
                in_flight_rest = self.calculator.calculate_min_in_flight_rest(max_fdp, rest_facility_class)
                if in_flight_rest:
                    result += f"Минимальный отдых в полете: {in_flight_rest}\n\n"

            # Проверяем лимиты
            planned_flight_time = timedelta(hours=2)  # Примерное время полета
            limits = self.calculator.check_limits(self.selected_crew_member_id, planned_flight_time,
                                                  sqlite3.connect('fdp_data.db'))
    ...
    def save_duty(self):
        ...
        try:
            ...
            if not self.selected_crew_member_id or not self.selected_aircraft_id:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (self.selected_crew_member_id, self.selected_aircraft_id, start_time, sectors,
                  departure, arrival, rest_in_flight, has_frms))
